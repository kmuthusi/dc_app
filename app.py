...
# disclaimer at bottom of sidebar (below upload control)
st.sidebar.markdown(
    """
    **Disclaimer:** *Football score prediction is highly uncertain and the
    results generated by this app are for learning and exploration only. Any
    financial decisions or bets placed based on these predictions are done at
    your own risk. The authors accept no responsibility for losses.*
    """
)
...
with tab_single:
    if not artifact:
        ...
        # warn if artifact seems to contain only dummy placeholder teams
        if len(teams) <= 1:
            st.warning(
                "Artifact contains only one team (placeholder). "
                "Replace with a trained league model for real team names."
            )
        if teams_current:
